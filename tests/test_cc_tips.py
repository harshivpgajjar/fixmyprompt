...
class CcTipGateTest(unittest.TestCase):
    ...
    def test_wellformed_new_work_gets_tip_only_block(self):
        ...
        # must offer a REAL send affordance, never a bare "press Enter" it can't
        # deliver. `[n ⏎]` always sends the original (via the on-disk one-shot
        # flag, independent of the clipboard).
