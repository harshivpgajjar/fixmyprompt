...
**Refiner:** subscription-native by default (a warm `claude` daemon, or the API path if you set a key), fail-open on any error/timeout (prompt just sends normally — the coach can never lock you out). Optionally reads `~/.claude/memory/core.md` and `design-taste.md` if present (this project's own optional personal-memory convention, not a Claude Code platform feature — harmless no-op for users who don't have these files) so refinement is voice- and taste-aware where that data exists, and is mode-aware everywhere (never "fixes" an explore prompt).
