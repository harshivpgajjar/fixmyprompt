# FixMyPrompt — HANDOFF (2026-07-05)
...
- Installed at `~/.claude/fixmyprompt` (runtime), source at `~/Desktop/fixmyprompt` (git repo). Hook wired into `~/.claude/settings.json`; `/refine` skill live; 911 real prompts backfilled into the report; weekly self-audit emits a "Prompting (FixMyPrompt)" section.
...
- **LIVE CONFIG (a 1-day trial):** `mode=always`, `use_daemon=true`, daemon running (pid was 53177, ~200MB) under LaunchAgent `com.fixmyprompt.daemon` (KeepAlive). `min_words=4`.

## Running processes
- **`com.fixmyprompt.daemon`** — warm `claude` stream-json session, KeepAlive LaunchAgent. This is a persistent SERVICE, not a batch run (no completion notification needed). It self-restarts on death; gate fails open to the local scaffold if it's down. Stop with `fixmyprompt daemon off`.
...
- Test isolation: CLI `try` tests must set `FIXMYPROMPT_HOME` to a temp dir or they read the user's live config.

## Revert
`fixmyprompt daemon off` (stops process + removes agent) then `fixmyprompt mode whisper` (or `off`).
